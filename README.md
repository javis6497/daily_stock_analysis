...
系统只输出量化研究信号和风险提示，不接券商、不自动下单，不构成保证收益或个人投顾建议。量化系统可以提高决策一致性和风险可见性，但不能保证稳定盈利。
...
- 盘前/盘后/基金报告顶部提供紧凑"操作要点"块：每只持仓一行动作·风险位·止盈位，并给出当日自选外候选的观察区与风险位，方便一眼抓住重点。
- 操作建议会生成组合总览、数据新鲜度检查、组合集中度和策略回测摘要。
- 回测采用逐日 walk-forward：每次只使用当时可见数据，信号下一根 K 线执行，并计入申购/赎回或买卖费、滑点和换手成本；输出相对基准、最大回撤、夏普和方向命中率。
- 自选外候选池按趋势、风险、市场环境、基金质量画像、A 股基本面质量画像和行业/主题分散打分，推送 Top N 候选观察。
- 14:00 盘中操作参考覆盖自选股/基金操作信号 + 盘中推荐候选，支持代理 ETF/指数的盘中估算，辅助 15:00 前人工决策。
- 盘后复盘会输出"今日推荐回顾"：汇总当天盘前/盘中推荐的自选外候选及其实际走势，复盘推荐质量。
...
- 报告会在 GitHub Actions 中归档为 artifact；JSON 台账包含策略版本、代码 SHA、数据截止日、完整指标和决策依据。
- 推荐使用独立 Cloudflare Worker 调度：工作日 08:37 盘前、14:07 基金操作提醒、16:37 盘后；周六/周日 09:37 周末量化周报。系统只允许在目标时间前后 5 分钟内发送。
- 每条钉钉消息的每个分片发送成功后立即写回执；重跑只补发未成功部分。钉钉网络错误和非零 API 错误码均会在时间窗内退避重试。
...
工作日 `fund_action` 的目标发送时间为北京时间 14:07，输出"盘中操作参考"：覆盖自选股/基金操作信号，并给出当日自选外候选推荐（观察区、风险位、理由），基金仍标注 15:00 前申购/赎回提醒。同一天成功发送后，后续同类触发会自动去重。
...
- `premarket`：盘前量化日报。
- `fund_action`：14:00 盘中操作参考（股票+基金+候选推荐）。
- `postmarket`：盘后量化复盘。
- `weekend_news`：周末量化周报。
