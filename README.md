...
- GitHub Actions 使用错峰时间触发：工作日 08:41/08:58/09:13/09:47 盘前，14:11/14:26/14:44 基金操作提醒，16:41/16:58/17:13 盘后；周六/周日 09:41/09:58/10:13 周末量化周报。
...
工作日 `fund_action` 会在北京时间 14:11 开始发送基金操作提醒，只包含自选基金/ETF，不包含股票、资讯或自选外候选，并会附带持仓逻辑跟踪和报告质检。若前一次未触发，14:26 和 14:44 会兜底尝试；同一天成功发送后会自动去重。

为降低 GitHub Actions 定时任务漏触发的影响，四类自动任务都会使用错峰触发点，避开整点、整点后 5 分钟内、15 分、30 分、45 分等常见高峰分钟，并用“北京时间日期 + session”的缓存标记跳过重复发送。盘前额外有 09:47 补发检查；手动触发 `workflow_dispatch` 不走去重限制，方便测试。
