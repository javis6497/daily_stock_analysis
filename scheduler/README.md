...
Worker 使用 UTC cron，配置已写在 `wrangler.toml`。注意 Cloudflare 免费版每个账号最多 **5 条 cron**，且使用 **ISO 星期编号（7=周日）**、不接受星期字段的逗号列表（如 `6,0`），所以周末拆成周六/周日两条。每个时段只触发一次，可靠性由 Worker 的 3 次 API 重试 + 仓库里的 1 条 GitHub 兜底 cron 共同保证。
